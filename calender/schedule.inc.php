...
if (preg_match("/schedule.inc.php/i", $_SERVER['PHP_SELF']))
{
...
{ 
	...
	class schedule_page extends base_calendar
	{
		...
		function createpage($current_day, $current_month, $current_year)
		{
			...
			{
					$this->pagelayout .= '<meta http-equiv="Content-Script-Type" content="text/javascript" />
...
<body leftmargin="0" topmargin="0" marginheight="0" marginwidth="0" onload="document.getElementById(\'eventlist\').scrollTop = '.$scrolltime.';">
';
					// require netscape schedule
					require ('schedule.ie6.php');
					$altschedule = new ie6_schedule_page;
					$this->pagelayout .= $altschedule->createpage($current_day, $current_month, $current_year);
					return;	
				}
				if ($browser_version == 2)
				{
					$this->pagelayout .= '<meta http-equiv="X-UA-Compatible" content="IE=8" />
...
<div style="height: 508px; width: 516px" id="eventlist">
			...
			for ($i = 0; $i < count($schedule_events); $i++)
			{
				if ($schedule_events[$i]['start_time'] == 'ALL')
				{
					...
					if ($timeblocks['ALL'][0] > 0)
						$blockwidth = intval(410 / $timeblocks['ALL'][0] - 7);
					else
						$blockwidth = 410;
					...
					else
					{
						if ($timeblocks['ALL']['left'] == $timeblocks['ALL']['right'])
							$timeblocks['ALL']['right'] = $blockleft + $blockwidth + 8;
						$timeblocks['ALL']['left'] = $blockleft + $blockwidth + 8;
					}
					$event_display = $schedule_events[$i]['long_event'];
					$this->pagelayout .= '<div class="event" style="top: '.(-1501 - $additional).'px; left: '.$blockleft.'px; height: '.$blockheight.'px; width: '.$blockwidth.'px" onclick="setEventDetails(\''.$event_display.'\')">
'.$schedule_events[$i]['short_event'].'
</div>
';
					$additional += $blockheight + 2;
				}
				else
				{
			...
			for ($i = 0; $i < $timecount; $i++)
			{
				...
				for ($j = 0; $j < $item_count; $j++)
				{
					...
					{
						$blockheight = (60 * $schedule_events[$j]['duration'] - 2);
						if ($schedule_events[$j]['block_cells'] > 0)
							$blockwidth = intval(410 / $schedule_events[$j]['block_cells'] - 7);
						else
							$blockwidth = 410;
						$blocktop = 59 + ($i * 15);
						...
						for ($s = 0; $s < $adding_time; $s = $s + 15)
						{
							...
							else
							{
								if ($timeblocks[$holdtime]['left'] == $timeblocks[$holdtime]['right'])
									$timeblocks[$holdtime]['right'] = $blockleft + $blockwidth + 8;
								$timeblocks[$holdtime]['left'] = $blockleft + $blockwidth + 8;
							}
						}
						$event_display = $schedule_events[$j]['long_event'];
						$this->pagelayout .= '<div class="event" style="top: '.(-1500 - $additional + $blocktop).'px; left: '.$blockleft.'px; height: '.$blockheight.'px; width: '.$blockwidth.'px" onclick="setEventDetails(\''.$event_display.'\')">
...
<td class="';
					...
					$this->pagelayout .= '" style="width: 154px; max-width: 154px; min-width: 154px; text-align: center; column-span: 7" nowrap="nowrap" colspan="7" onclick="showCalendar('.$onmonth.', '.$onyear.')">
...
<div style="height: 250px; width: 314px">
