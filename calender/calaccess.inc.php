...
if (preg_match("/calaccess.inc.php/i", $_SERVER['PHP_SELF']))
{
...
{ 
	...
	class caldbaccess extends DB_Sql
	{
		...
		{
			$a = $this->tbl_recurring;
			$query = "SELECT * FROM $a WHERE userid = '".mysql_real_escape_string($userid)."'";
			$this->query($query);
			return $this->affected_rows();
		}
		...
		function AddByDate($evdate, $title, $detail, $duration, $userid)
		{
			...
			$query .= "VALUES ($evdate, '".mysql_real_escape_string($title)."', '".mysql_real_escape_string($detail)."', $duration, '".mysql_real_escape_string($userid)."')";
		...
		function AddRecurring($weekday, $evtime, $period, $month, $title, $detail, $duration, $userid)
		{
			...
			$query .= "VALUES ($weekday, '".mysql_real_escape_string($evtime)."', $period, $month, '".mysql_real_escape_string($title)."', '".mysql_real_escape_string($detail)."', $duration, '".mysql_real_escape_string($userid)."')";
			return $this->query($query);
		}
		...
		function SQLLogin($userid, $password)
		{
			...
			$query = "SELECT userid, level FROM $a WHERE userid='".mysql_real_escape_string($userid)."' AND password='".mysql_real_escape_string($password)."'";
		...
		function GetUserData($userid)
		{
			$a = $this->tbl_users;
			$userid = $this->RemoveJunkTags($userid);
			$query = "SELECT userid, level FROM $a WHERE userid='".mysql_real_escape_string($userid)."'";
		...
		function AddNewUser($accountid, $accesslevel, $accountpassword)
		{
			...
			if (!$accountpassword)
				$query = "INSERT INTO $a (userid, level) VALUES ('".mysql_real_escape_string($accountid)."', $accesslevel)";
			else
			{
				$accountpassword = $this->RemoveJunkTags($accountpassword);
				$query = "INSERT INTO $a (userid, password, level) VALUES ('".mysql_real_escape_string($accountid)."', '".mysql_real_escape_string($accountpassword)."', $accesslevel)";
			}
			return $this->query($query);
		}
		...
		function UpdateUser($accountid, $existingid, $accesslevel, $accountpassword)
		{
			...
			if (!$accountpassword)
				$query = "UPDATE $a SET userid='".mysql_real_escape_string($accountid)."', level=$accesslevel WHERE userid='".mysql_real_escape_string($existingid)."'";
			else
			{
				$accountpassword = $this->RemoveJunkTags($accountpassword);
				$query = "UPDATE $a SET userid='".mysql_real_escape_string($accountid)."', password='".mysql_real_escape_string($accountpassword)."', level=$accesslevel WHERE userid='".mysql_real_escape_string($existingid)."'";
			}
			$this->query($query);
		}
		...
		function DeleteUser($accountid)
		{
			...
			$query = "DELETE FROM $a WHERE userid='".mysql_real_escape_string($accountid)."'";
			$this->query($query);
		}
	}
}
