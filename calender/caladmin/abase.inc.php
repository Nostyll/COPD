...
if (preg_match("/abase.inc.php/i", $_SERVER['PHP_SELF']))
{
...
';
		...
		function deletecalendarevent()
		{
			...
			if (isset($_POST['eventid']))
			{
				// create the database access object
				$accesscaldb = new caldbaccess;
				$itemid = stripslashes($_POST['eventid']);
				if (strstr($itemid, "D"))
				{
		...
		function addnewcalendaruser()
		{
			...
			{
				$accountid = strtolower(trim(stripslashes($_POST['useraccount'])));
				// create the setup object
				$conf = new layout_setup;
				if ((!isset($_POST['accountpassword']) && !$conf->admin_serverside_login) || !isset($_POST['accountlevel']))
					return $lang->word_create_fail;
				$accesslevel = stripslashes($_POST['accountlevel']);
				$accesspass = FALSE;
				if (!$conf->admin_serverside_login)
					$accesspass = trim(stripslashes($_POST['accountpassword']));
				unset($conf);
		...
		function updatecalendaruser()
		{
			...
			{
				$existingid = strtolower(trim(stripslashes($_POST['editaccount'])));
				// create the setup object
				$conf = new layout_setup;
				if ((!isset($_POST['accountpassword']) && !$conf->admin_serverside_login) || !isset($_POST['accountlevel']))
					return $lang->word_create_fail;
