...
if (preg_match("/schedule.ie6.php/i", $_SERVER['PHP_SELF']))
{
...
if(!defined("IE6_SCHEDULE_PAGE")) 
{ 
	define("IE6_SCHEDULE_PAGE", TRUE); 

	require ('setup.inc.php');
	require ('language.inc.php');
	require ('calaccess.inc.php');

	class ie6_schedule_page extends base_calendar
	{
		// Constructor
		function ie6_schedule_page()
		{
			$this->alternate_page = '';
			$this->activeday = 0;
			$this->activemonth = 0;
...
<a class="calmenu" style="font-size: 10px" href="http://www.iamwhen.com" target="_blank">Web Calendar &copy;2009 Proverbs, LLC. All rights reserved.</a>
</center>';
		...
		function makeschedule($current_day, $current_month, $current_year)
		{
			...
			$this->alternate_page .= '<td style="padding: 0px; text-align: right">
...
', '<br />', $calaccess->f('long_description'));
			...
			for ($i = 0; $i < $item_count; $i++)
			{
				if ($schedule_events[$i]['start_time'] == 'ALL')
				{
					...
					$this->alternate_page .= '<div class="event" style="top: 1px; left: '.$blockleft.'px; height: '.$blockheight.'px; width: '.$blockwidth.'px" onclick="setEventDetails(\''.$event_display.'\')">
			...
			for ($i = 0; $i < $timecount; $i++)
			{
				...
				for ($j = 0; $j < $item_count; $j++)
				{
					if ($schedule_events[$j]['start_time'] == $on_time)
					{
						...
						$this->alternate_page .= '<div class="event" style="top: '.$blocktop.'px; left: '.$blockleft.'px; height: '.$blockheight.'px; width: '.$blockwidth.'px" onclick="setEventDetails(\''.$event_display.'\')">
...
';
		...
		function makeminicals($current_day, $current_month, $current_year)
		{
			// create the language object
			$lang = new language;
			$this->alternate_page .= '<td style="padding: 0px">
<table cellspacing="0" cellpadding="0" border="0">
<tr>
...
<td class="';
					if ($onyear == $current_year && $onmonth == $current_month)
						$this->alternate_page .= 'aminical';
					else
						$this->alternate_page .= 'minical';
					$this->alternate_page .= '" style="width: 154px; max-width: 154px; min-width: 154px; text-align: center; column-span: 7" nowrap="nowrap" colspan="7" onclick="showCalendar('.$onmonth.', '.$onyear.')">
...
';
					for ($d = 0; $d < 7; $d++)
					{
						$this->alternate_page .= '<td class="minical" style="text-align: center; cursor: default">
'.$lang->day_init[$d].'
</td>
';	
					}
						...
						for ($j = 0; $j < 7; $j++)
						{
							if (($calday == 1 && $firstweekday == $j) || ($calday > 1 && $calday <= $lastday))
							{
								if ($onyear == $current_year && $onmonth == $current_month && $calday == $current_day)
									$this->alternate_page .= '<td class="aminical" ';
								else
									$this->alternate_page .= '<td class="minical" ';
								$this->alternate_page .= 'onclick="showSchedule('.$calday.', '.$onmonth.', '.$onyear.')">
';
								if ($calaccess->CheckForEvents($calday, $onmonth, $onyear))
									$this->alternate_page .= '<b>'.$calday.'</b>';
								else
									$this->alternate_page .= $calday;
								$this->alternate_page .= '&nbsp;
</td>
';
								$calday++;
								...
								$this->alternate_page .= '<td class="minical" style="cursor: default">
&nbsp;
</td>
';
						}
